...
Original file is located at
    https://colab.research.google.com/drive/1KQ8GFNlM8ZnnOzyZabh05DJcJAkK9psL
...
# Google Drive
!gdown --id '1G9OkRwHAX0yuGr8Z7glEECPUCj5xT4L5' --output food-11.zip
...
def loss_fn_kd(outputs, labels, teacher_outputs, alpha=0.5):
    ...
    soft_loss = nn.KLDivLoss(reduction = 'batchmean')(F.log_softmax(outputs/temperature, dim=1),F.softmax(teacher_outputs/temperature, dim=1))
    return hard_loss + soft_loss * alpha * (temperature ** 2)
...
optimizer = torch.optim.Adam(student_net.parameters(), lr=2e-4, weight_decay=2e-5)
