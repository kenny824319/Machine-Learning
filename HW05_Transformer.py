...
Original file is located at
    https://colab.research.google.com/drive/1gO4Dwpp0g8Jo3OjlzEHJwHu2OxxVyBUb
...
def clean(prefix, l1, max_len=1000, min_len=1):
    with open(f'{prefix}/ted_zh_corpus.deduped', 'r') as l1_in_f:
      with open(f'{prefix}/ted_zh_corpus.deduped.clean.{l1}', 'w') as l1_out_f:
        for s1 in l1_in_f:
          s1 = s1.strip()
          s1 = clean_s(s1, l1)
          s1_len = len_s(s1, l1)
...
with open(f'{mono_prefix}/mono.tok.zh', 'w') as out_f:
  with open(f'{mono_prefix}/ted_zh_corpus.deduped.clean.zh', 'r') as in_f:
    for line in in_f:
      line = line.strip()
      tok = spm_model.encode(line, out_type=str)
      print(' '.join(tok), file=out_f)
...
with open(f'{mono_prefix}/mono.tok.en', 'w') as f:
	for i in range(781713):
		f.write('{}\n'.format('。'))
...
generate_prediction(model, task, split='mono' ,outfile="/content/mono.en.txt")
...
with open(f'{mono_prefix}/mono.finaltok.en', 'w') as out_f:
  with open('/content/mono.en.txt', 'r') as in_f:
    for line in in_f:
      line = line.strip()
      tok = spm_model.encode(line, out_type=str)
      print(' '.join(tok), file=out_f)

!cp /content/DATA/rawdata/mono/mono.tok.zh /content/DATA/rawdata/mono/mono.finaltok.zh
...
src_dict_file = './DATA/data-bin/ted2020/dict.en.txt'
tgt_dict_file = src_dict_file
monopref = './DATA/rawdata/mono/mono.finaltok' # or whatever path after applying subword tokenization, w/o the suffix (.zh/.en)
if binpath.exists():
    print(binpath, "exists, will not overwrite!")
else:
    !python -m fairseq_cli.preprocess\
...
config = Namespace(
    ...
    savedir = "/content/drive/MyDrive/ML2021-hw5/checkpoints/transformer-bt",
    source_lang = "en",
    target_lang = "zh",
    ...
    # checkpoints
    keep_last_epochs=5,
    resume="final_checkpoint_last.pt", # if resume from checkpoint name (under config.savedir)
...
generate_prediction(model, task, outfile="/content/drive/MyDrive/ML2021-hw5/prediction.txt")
